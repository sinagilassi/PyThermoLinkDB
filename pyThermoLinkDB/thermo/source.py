...
class Source:
    ...
    def matX(
        ...
        Notes
        -----
        - The mixture name is generated by sorting the components and joining their identifiers using the specified mixture_key and delimiter.
        - If mixture_key is not provided, self.mixture_key is used.
        - The method relies on the matrix_data_extractor to retrieve the TableMatrixData object for the generated mixture name and property name.
        - If the matrix_data_extractor returns None, this method will also return None.
        - matrix data is extracted using the matX method according to the order of the components provided which might be different from the order used to generate the mixture name.
        '''
        # determine mixture key to use
        selected_mixture_key = mixture_key if mixture_key is not None else self.mixture_key

        # generate mixture name from components (alphabetically sorted) using the selected mixture key
